...
#		fd = open("log.txt",'w')
	...
	def one_hand_gesture_recognition(self, hand):
		if hand.grab_strength < 0.5:
			...
			print("{},{},{}".format(cmd_dict ['normal_x'],cmd_dict ['normal_y'],cmd_dict ['normal_z']))
#			fd = open("log.txt",'w')
#			fd.write( json.dumps(cmd_dict))
			#send_data += '&'
			self.client.send(send_data)
		elif hand.grab_strength > 0.85:
			normal = hand.palm_normal
			velocity = hand.palm_velocity
			cmd_dict = {'action': 'close', 'normal_x': normal.x, 'normal_y': normal.y, 'normal_z': normal.z, 'velocity_x': velocity.x, 'velocity_y': velocity.y, 'velocity_z': velocity.z}
			send_data = json.dumps(cmd_dict)
                        print cmd_dict['action']
			self.client.send(send_data)
